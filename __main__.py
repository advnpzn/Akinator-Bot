...
async def aki_play_cmd_handler(update: Update, context: CallbackContext) -> None:
    ...
    q = aki.start_game(language=getLanguage(user_id), child_mode=bool(getChildMode(user_id)))
...
async def aki_play_callback_handler(update: Update, context: CallbackContext) -> None:
    user_id = update.effective_user.id
    aki: akipy.Akinator = context.user_data[f"aki_{user_id}"]
    ...
    if a == '5':
        ...
        try:
            ...
            await query.answer(text="Akinator timed out, please try again", show_alert=True)
    else:
        try:
            q = aki.answer(a)
        except Exception:
    ...
    if not aki.win:
        await query.message.edit_media(
            InputMediaPhoto(
                media=f"https://en.akinator.com/assets/img/akitudes_670x1096/{aki.akitude}",
                caption=aki.question,
            ),
            reply_markup=AKI_PLAY_KEYBOARD
        )
        ...
        await query.message.edit_media(
            InputMediaPhoto(media=aki.photo,
                            caption=f"It's {aki.name_proposition} ({aki.description_proposition})! Was I correct?"
                            ),
